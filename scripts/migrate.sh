...
get_current_revision() {
    ...
    if [ "$USE_DOCKER" = true ]; then
        revision=$(docker exec "$API_CONTAINER" bash -c "cd /app && PYTHONPATH=/app/src uv run --no-sync python -m alembic current 2>/dev/null" | grep -oE '[a-f0-9]{12}' | head -1)
    else
...
generate_migration_sql() {
    ...
    if [ "$USE_DOCKER" = true ]; then
        docker exec "$API_CONTAINER" bash -c "cd /app && PYTHONPATH=/app/src uv run --no-sync python -m alembic upgrade $target --sql" > "$sql_file" 2>&1
    else
...
run_test_migration() {
    ...
    if [ "$USE_DOCKER" = true ]; then
        if timeout "$MIGRATION_TIMEOUT" docker exec "$API_CONTAINER" bash -c \
            "cd /app && DATABASE_URL='$test_db_url' PYTHONPATH=/app/src uv run --no-sync python -m alembic upgrade $TARGET_REVISION"; then
...
apply_migration() {
    ...
    if [ "$USE_DOCKER" = true ]; then
        if timeout "$MIGRATION_TIMEOUT" docker exec "$API_CONTAINER" bash -c \
            "cd /app && PYTHONPATH=/app/src uv run --no-sync python -m alembic $COMMAND $TARGET_REVISION"; then
...
validate_migration_state() {
    ...
    if [ "$USE_DOCKER" = true ]; then
        local pending=$(docker exec "$API_CONTAINER" bash -c "cd /app && PYTHONPATH=/app/src uv run --no-sync python -m alembic history -r current:head" | grep -c "Rev:")
    else
...
handle_current() {
    ...
    if [ "$USE_DOCKER" = true ]; then
        docker exec "$API_CONTAINER" bash -c "cd /app && PYTHONPATH=/app/src uv run --no-sync python -m alembic history -r $revision:$revision -v" 2>/dev/null
    else
...
handle_history() {
    ...
    if [ "$USE_DOCKER" = true ]; then
        docker exec "$API_CONTAINER" bash -c "cd /app && PYTHONPATH=/app/src uv run --no-sync python -m alembic history -v"
    else
